...
const tasks = [
  {
    ...
    desc: "Follow @SwappedNFT, then return and confirm your X username.",
    cta: "Follow on X",
    icon: "bell",
...
export const ApplyModal = ({ open, onClose }) => {
  ...
  const handleOpenAction = (task) => {
    if (task.id !== 1 && state[1] !== "done") {
      toast.error("Please complete Follow us on X first");
      return;
    }
    ...
    if (task.id === 1) {
      ...
      toast.info("After following, return here and enter your X username");
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-9 h-9 rounded-full flex items-center justify-center border border-[#F2C829]/40 text-[#F4EBCC]"
        >
          <X size={18} />
        </button>
          ...
          {tasks.map((t) => {
            ...
            return (
              <div key={t.id} className={`rounded-xl bg-[#16291A] border border-[#F2C829]/20 p-3.5 ${isLocked ? "opacity-50" : ""}`}>
                <div className="flex items-center justify-between gap-3">
                  ...
                  {!isDone && (
                    <button
                      ...
                      disabled={isLocked}
                      className="btn-press inline-flex items-center gap-1.5 text-[12px] font-bold px-3.5 py-2 rounded-lg bg-yellow-brand brand-forest disabled:opacity-50"
                    >
                      {isLocked ? "Locked" : t.cta}
                ...
                {t.id === 1 && showUsernameInput && !isDone && (
            ...
            <>
              ...
              <div className="w-full max-w-md mx-auto flex flex-col gap-3 mt-4">
                ...
                <button
                  ...
                  className="btn-press inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg bg-yellow-brand brand-forest font-bold"
                >
                  Submit Whitelist
                </button>
              </div>
